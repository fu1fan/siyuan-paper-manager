...
/** 右下角状态栏的翻译进度指示；数据来自 pdf2zh 文本输出解析。 */
export function mountTranslationStatusBar(plugin: Plugin, status: StatusStore): () => void {
  ...
  addStatusBar.call(plugin, { element, position: "right" });
...
function render(element: HTMLElement, translation: TranslationState): void {
  switch (translation.state) {
    case "running": {
      const percent = translation.progress != null ? ` ${translation.progress}%` : "";
      element.textContent = `⏳ 翻译中${percent}`;
