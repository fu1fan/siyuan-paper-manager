...
打开任一论文文献库时，插件按文档 ID 核对数据库完整成员与直属子笔记，不受数据库视图筛选影响，不比较标题、作者等元数据。发现差异后列出“待删除数据库条目”和“待添加到数据库的子笔记”，默认不勾选。应用只移除数据库行或绑定已有笔记，不删除实际文档，也不自动补写详细元数据。应用前会再次核对，已变化的条目自动跳过。

“本次忽略勾选项”只影响当前弹窗，重新打开文献库仍可提醒；“勾选项不再提醒”按文献库与异常类型保存在对应文档的 `custom-paper-membership-ignored` 隐藏属性中。文档已删除或数据库行未绑定文档时，记录保存在文献库同名属性中。清除相应属性记录后恢复提醒。
