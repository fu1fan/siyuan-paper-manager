...
- `src/ui`：首次向导、五组设置、命令、顶栏、右键菜单、元数据与引用导出对话框。
...
- 文献库文档通过 `custom-paper-library-data` 保存 AV 与字段标识。
- 论文文档通过 `custom-paper-library-id` 确定权威归属，数据库行绑定论文文档块。
- 元数据和项目从论文 base64 投影到数据库；全量同步会恢复缺失行、删除失效绑定并覆盖被手改的插件列。
- 每个文献库独立配置插件字段；用户自建数据库列不在插件删除范围内。
...
6. 写 `custom-paper-data`、文献库归属和索引属性。
7. 将论文文档绑定到默认文献库数据库并投影所选字段。
8. 使用插件内置模板引擎渲染打包模板，不向 `data/templates` 写入文件。
